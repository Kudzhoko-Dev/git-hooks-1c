...
def compile_(input_folder: Path, output_file: Path):
    temp_source_path = Path(tempfile.mkdtemp())
    if not temp_source_path.exists():
        temp_source_path.mkdir(parents=True)
    else:
        shutil.rmtree(str(temp_source_path), ignore_errors=True)

    renames_file = input_folder / 'renames.txt'

    with renames_file.open(encoding='cp866') as read:
        for line in read:
            print(line)
